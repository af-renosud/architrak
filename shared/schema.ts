...
// Task #661 — PV package/reception workflow. Cross-table guards are SQL-only.
export const marcheLotAssignments = pgTable("marche_lot_assignments", {
  id: serial("id").primaryKey(),
  marcheId: integer("marche_id").notNull().references(() => marches.id, { onDelete: "cascade" }),
  lotId: integer("lot_id").notNull().references(() => lots.id, { onDelete: "cascade" }),
  assignedBy: text("assigned_by").notNull(), assignedAt: timestamp("assigned_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
  removedBy: text("removed_by"), removedAt: timestamp("removed_at"), source: text("source").notNull().default("manual"), note: text("note"),
}, (t) => [uniqueIndex("marche_lot_assignments_active_unique").on(t.marcheId, t.lotId).where(sql`${t.removedAt} IS NULL`), check("marche_lot_assignments_source_chk", sql`${t.source} IN ('manual', 'devis_suggestion')`), check("marche_lot_assignments_removed_audit_chk", sql`(${t.removedAt} IS NULL) = (${t.removedBy} IS NULL)`)]);
export const pvPackages = pgTable("pv_packages", {
  id: serial("id").primaryKey(), projectId: integer("project_id").notNull().references(() => projects.id, { onDelete: "cascade" }), marcheId: integer("marche_id").notNull().references(() => marches.id), contractorId: integer("contractor_id").notNull().references(() => contractors.id),
  reference: text("reference").notNull(), receptionKind: text("reception_kind").notNull(), receptionDate: date("reception_date").notNull(), status: text("status").notNull().default("draft"), source: text("source").notNull().default("native"), supersedesPackageId: integer("supersedes_package_id"), version: integer("version").notNull().default(1),
  createdBy: text("created_by").notNull(), createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(), updatedBy: text("updated_by"), updatedAt: timestamp("updated_at").default(sql`CURRENT_TIMESTAMP`).notNull(), confirmedBy: text("confirmed_by"), confirmedAt: timestamp("confirmed_at"), supersededBy: text("superseded_by"), supersededAt: timestamp("superseded_at"),
}, (t) => [index("pv_packages_project_id_idx").on(t.projectId), index("pv_packages_marche_id_idx").on(t.marcheId), check("pv_packages_reception_kind_chk", sql`${t.receptionKind} IN ('partial', 'final')`), check("pv_packages_status_chk", sql`${t.status} IN ('draft', 'confirmed', 'sent_for_signature', 'signed', 'delivered', 'superseded', 'legacy')`), check("pv_packages_source_chk", sql`${t.source} IN ('native', 'legacy_import')`), check("pv_packages_version_positive_chk", sql`${t.version} > 0`), check("pv_packages_confirmed_audit_chk", sql`${t.status} NOT IN ('confirmed', 'sent_for_signature', 'signed', 'delivered', 'superseded') OR (${t.confirmedBy} IS NOT NULL AND ${t.confirmedAt} IS NOT NULL)`), check("pv_packages_superseded_audit_chk", sql`${t.status} != 'superseded' OR (${t.supersededBy} IS NOT NULL AND ${t.supersededAt} IS NOT NULL)`)]);
export const pvPackageLots = pgTable("pv_package_lots", { packageId: integer("package_id").notNull().references(() => pvPackages.id, { onDelete: "cascade" }), lotId: integer("lot_id").notNull().references(() => lots.id), assignmentId: integer("assignment_id").notNull().references(() => marcheLotAssignments.id), addedAt: timestamp("added_at").default(sql`CURRENT_TIMESTAMP`).notNull() }, (t) => [unique("pv_package_lots_package_lot_unique").on(t.packageId, t.lotId)]);
export const pvPackageDocuments = pgTable("pv_package_documents", {
  id: serial("id").primaryKey(), packageId: integer("package_id").notNull().references(() => pvPackages.id, { onDelete: "cascade" }), documentKind: text("document_kind").notNull(), displayOrder: integer("display_order").notNull().default(0), title: text("title").notNull(), originalFilename: text("original_filename").notNull(), originalStorageKey: text("original_storage_key").notNull(), originalMimeType: text("original_mime_type").notNull(), originalSha256: text("original_sha256").notNull(), originalSizeBytes: integer("original_size_bytes").notNull(), pdfStorageKey: text("pdf_storage_key").notNull(), pdfSha256: text("pdf_sha256").notNull(), pdfPageCount: integer("pdf_page_count").notNull(), uploadedBy: text("uploaded_by").notNull(), uploadedAt: timestamp("uploaded_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (t) => [uniqueIndex("pv_package_documents_one_base_unique").on(t.packageId).where(sql`${t.documentKind} = 'base'`), uniqueIndex("pv_package_documents_annex_order_unique").on(t.packageId, t.displayOrder).where(sql`${t.documentKind} = 'annex'`), check("pv_package_documents_kind_chk", sql`${t.documentKind} IN ('base', 'annex')`), check("pv_package_documents_display_order_chk", sql`${t.displayOrder} >= 0`), check("pv_package_documents_original_sha256_chk", sql`${t.originalSha256} ~ '^[0-9a-f]{64}$'`), check("pv_package_documents_pdf_sha256_chk", sql`${t.pdfSha256} ~ '^[0-9a-f]{64}$'`), check("pv_package_documents_size_chk", sql`${t.originalSizeBytes} > 0 AND ${t.pdfPageCount} > 0`)]);
export const pvReserves = pgTable("pv_reserves", { id: serial("id").primaryKey(), projectId: integer("project_id").notNull().references(() => projects.id, { onDelete: "cascade" }), marcheId: integer("marche_id").references(() => marches.id), contractorId: integer("contractor_id").references(() => contractors.id), title: text("title").notNull(), description: text("description"), severity: text("severity").notNull(), status: text("status").notNull().default("open"), owner: text("owner"), dueDate: date("due_date"), version: integer("version").notNull().default(1), createdBy: text("created_by").notNull(), createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(), updatedBy: text("updated_by"), updatedAt: timestamp("updated_at").default(sql`CURRENT_TIMESTAMP`).notNull() }, (t) => [index("pv_reserves_project_id_idx").on(t.projectId), check("pv_reserves_severity_chk", sql`${t.severity} IN ('observation', 'minor', 'major', 'blocking')`), check("pv_reserves_status_chk", sql`${t.status} IN ('open', 'resolved', 'waived')`), check("pv_reserves_version_positive_chk", sql`${t.version} > 0`)]);
export const pvReserveLots = pgTable("pv_reserve_lots", { reserveId: integer("reserve_id").notNull().references(() => pvReserves.id, { onDelete: "cascade" }), lotId: integer("lot_id").notNull().references(() => lots.id) }, (t) => [unique("pv_reserve_lots_reserve_lot_unique").on(t.reserveId, t.lotId)]);
export const pvReserveEvents = pgTable("pv_reserve_events", { id: serial("id").primaryKey(), reserveId: integer("reserve_id").notNull().references(() => pvReserves.id, { onDelete: "cascade" }), fromStatus: text("from_status"), toStatus: text("to_status"), comment: text("comment"), payload: jsonb("payload"), actor: text("actor").notNull(), createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull() });
export const pvPackageRevisions = pgTable("pv_package_revisions", { id: serial("id").primaryKey(), packageId: integer("package_id").notNull().references(() => pvPackages.id, { onDelete: "cascade" }), revisionNumber: integer("revision_number").notNull(), manifest: jsonb("manifest").notNull(), manifestSha256: text("manifest_sha256").notNull(), pdfStorageKey: text("pdf_storage_key").notNull(), pdfSha256: text("pdf_sha256").notNull(), pdfPageCount: integer("pdf_page_count").notNull(), confirmedBy: text("confirmed_by").notNull(), confirmedAt: timestamp("confirmed_at").notNull() }, (t) => [unique("pv_package_revisions_package_revision_unique").on(t.packageId, t.revisionNumber), check("pv_package_revisions_number_positive_chk", sql`${t.revisionNumber} > 0`), check("pv_package_revisions_manifest_sha256_chk", sql`${t.manifestSha256} ~ '^[0-9a-f]{64}$'`), check("pv_package_revisions_pdf_sha256_chk", sql`${t.pdfSha256} ~ '^[0-9a-f]{64}$'`), check("pv_package_revisions_page_count_chk", sql`${t.pdfPageCount} > 0`)]);
export const pvRevisionReserves = pgTable("pv_revision_reserves", { revisionId: integer("revision_id").notNull().references(() => pvPackageRevisions.id, { onDelete: "cascade" }), reserveId: integer("reserve_id").notNull().references(() => pvReserves.id), reserveSnapshot: jsonb("reserve_snapshot").notNull(), snapshotSha256: text("snapshot_sha256").notNull() }, (t) => [unique("pv_revision_reserves_revision_reserve_unique").on(t.revisionId, t.reserveId), check("pv_revision_reserves_snapshot_sha256_chk", sql`${t.snapshotSha256} ~ '^[0-9a-f]{64}$'`)]);
export const pvPackageEvents = pgTable("pv_package_events", { id: serial("id").primaryKey(), packageId: integer("package_id").notNull().references(() => pvPackages.id, { onDelete: "cascade" }), eventType: text("event_type").notNull(), payload: jsonb("payload"), actor: text("actor").notNull(), createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull() });
...
export type MarcheLotAssignment = typeof marcheLotAssignments.$inferSelect;
export type PvPackage = typeof pvPackages.$inferSelect;
export type PvPackageLot = typeof pvPackageLots.$inferSelect;
export type PvPackageDocument = typeof pvPackageDocuments.$inferSelect;
export type PvReserve = typeof pvReserves.$inferSelect;
export type PvReserveLot = typeof pvReserveLots.$inferSelect;
export type PvReserveEvent = typeof pvReserveEvents.$inferSelect;
export type PvPackageRevision = typeof pvPackageRevisions.$inferSelect;
export type PvRevisionReserve = typeof pvRevisionReserves.$inferSelect;
export type PvPackageEvent = typeof pvPackageEvents.$inferSelect;
