...
## Correction (2026-04-23, post-recovery)

The "Numbers" and "Timeline" sections above hypothesised the 8 missing tracker rows would be tags **0011–0018**. When the production deploy crash-looped this week the boot invariant from Task #136 named `0015_little_ender_wiggin` as the offender, and a read-only query against the production replica confirmed the actual missing range was **0015–0022** (not 0011–0018). The future-dated-`when` mechanism still cleanly explains the gap — the boundary just sits four entries later than originally hypothesised, because tracker rows for 0011–0014 were already in place from earlier successful deploys before the future-dated `when` values took effect. Recovery via `scripts/reconcile-drizzle-tracker.ts --apply` (Task #142) inserted the 8 missing rows for 0015–0022 in a single transaction; the post-recovery tracker count is 23/23 and the schema-presence boot invariant now passes.
