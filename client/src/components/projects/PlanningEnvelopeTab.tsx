  ...
  return (
    <div className="space-y-4" data-testid="panel-planning-envelope">
      ...
      {!data?.revisions?.length ? <LuxuryCard className="py-12 text-center" data-testid="planning-envelope-empty"><FilePlus2 size={23} className="mx-auto text-[#b9784c]" /><p className="mt-3 text-sm font-semibold">No planning candidates yet</p><p className="mt-1 text-xs text-muted-foreground">Create a revision manually or import a contractor PDF to establish the first working envelope.</p></LuxuryCard> :
        <div className="space-y-3">{data.revisions.map((item) => <RevisionCard key={item.revision.id} item={item} revisions={data.revisions} projectId={projectId} isArchived={isArchived} isRescraping={rescrape.isPending && rescrape.variables?.id === item.revision.id} onEdit={() => { setEditing(item); setDialog("edit"); }} onReview={() => { setEditing(item); setDialog("review"); }} onAction={(url, body) => action.mutate({ url, body })} onRescrape={() => rescrape.mutate({ id: item.revision.id, expectedVersion: item.revision.version })} onPromote={() => setPromote(item)} onViewPdf={() => setPdfRevision(item)} />)}</div>}
...
function RevisionCard({ item, revisions, projectId, isArchived, isRescraping, onEdit, onReview, onAction, onRescrape, onPromote, onViewPdf }: { item: Revision; revisions: Revision[]; projectId: string; isArchived: boolean; isRescraping: boolean; onEdit: () => void; onReview: () => void; onAction: (url: string, body?: unknown) => void; onRescrape: () => void; onPromote: () => void; onViewPdf: () => void }) {
