...
import { PlanningLineReviewWorkspace, type PlanningLineReview } from "./PlanningLineReviewWorkspace";
import {
...
type Revision = { revision: { id: number; status: "draft" | "reviewed" | "approved" | "superseded"; reference: string; descriptionFr: string; documentDate?: string | null; amountHt: string; amountTtc: string; tvaRatePercent?: string | null; tvaAutoliquidation?: boolean; version: number; contractorId?: number | null; lotId?: number | null; archidocTechnicalLotId?: string | null; supersedesRevisionId?: number | null; promotedDevisId?: number | null; promotedAt?: string | null; updatedAt: string }; lines: Line[]; lineReviews?: PlanningLineReview[]; source: { sourceKind: "manual" | "pdf_upload"; fileName?: string | null; fileSha256?: string | null; confidence?: number | null; warnings?: { message?: string; severity?: string }[]; requiresVerification?: boolean; verifiedAt?: string | null; verificationNote?: string | null } | null; contractorName: string | null; lotNumber: string | null; technicalLot: TechnicalLot | null; legacyLotNeedsReview?: boolean };
type PlanningImport = { id: number; fileName: string; fileSha256: string; status: "processing" | "succeeded" | "failed" | "stale"; stage: "accepted" | "extracting" | "validating" | "storing" | "saving" | "complete"; revisionId: number | null; errorCode: string | null; errorMessage: string | null; startedAt: string; updatedAt: string; completedAt: string | null };
...
export function PlanningEnvelopeTab({ projectId, contractors, isArchived, floorPlan }: Props) {
  ...
  const rescrape = useMutation({
    mutationFn: async ({ id, expectedVersion }: { id: number; expectedVersion: number }) => {
      const response = await apiRequest("POST", `/api/planning-revisions/${id}/rescrape`, { expectedVersion });
      return response.json();
    },
    onSuccess: (extracted) => {
      invalidate();
      const revision = extracted?.revision
        ? {
            revision: extracted.revision,
            lines: extracted.lines ?? [],
            source: extracted.source ?? null,
            contractorName: extracted.contractorName ?? null,
            lotNumber: extracted.lotNumber ?? null,
            technicalLot: extracted.technicalLot ?? null,
          } as Revision
        : null;
      if (revision) {
        setEditing(revision);
      ...
      toast({
        title: "PDF re-scraped",
        description: "A new draft was created with the current extraction rules. Review its lines before approving.",
      });
    },
    onError: (e: Error) => toast({ title: "PDF could not be re-scraped", description: e.message, variant: "destructive" }),
  });
  const deleteDraft = useMutation({
    mutationFn: async ({ id, expectedVersion }: { id: number; expectedVersion: number }) => {
      const response = await apiRequest("DELETE", `/api/planning-revisions/${id}`, { expectedVersion });
      return response.json();
    },
    onSuccess: () => {
      const deletedId = deleteRevision?.revision.id;
      invalidate();
    ...
    onError: (e: Error) => toast({
      title: "Candidate quotation could not be deleted",
  ...
  return (
    <div className="space-y-4" data-testid="panel-planning-envelope">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={(event) => {
                ...
                if (deleteRevision) {
                  deleteDraft.mutate({
                    id: deleteRevision.revision.id,
                    expectedVersion: deleteRevision.revision.version,
                  });
                }
              }}
      ...
      {pdfRevision && (
        <PdfPopoutViewer
          viewerId={`planning-${pdfRevision.revision.id}`}
          devisCode={
            pdfRevision.source?.fileName ??
            pdfRevision.revision.reference ??
            `Planning revision ${pdfRevision.revision.id}`
          }
          pdfUrl={`/api/planning-revisions/${pdfRevision.revision.id}/pdf`}
