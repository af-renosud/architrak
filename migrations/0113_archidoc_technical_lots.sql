-- ArchiDoc Settings → Lots Techniques read-only mirror for Planning.
-- Project lots remain a separate contractual concept.

CREATE TABLE IF NOT EXISTS archidoc_technical_lots (
  archidoc_id varchar(255) PRIMARY KEY,
  code text NOT NULL,
  label_fr text NOT NULL,
  display_order integer NOT NULL,
  is_active boolean NOT NULL,
  deleted_at timestamp,
  archidoc_created_at timestamp NOT NULL,
  archidoc_updated_at timestamp NOT NULL,
  source_base_url text,
  synced_at timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT archidoc_technical_lots_display_order_chk CHECK (display_order >= 0),
  CONSTRAINT archidoc_technical_lots_lifecycle_chk CHECK (
    (is_active = true AND deleted_at IS NULL) OR is_active = false
  )
);--> statement-breakpoint

CREATE INDEX IF NOT EXISTS archidoc_technical_lots_selection_idx
  ON archidoc_technical_lots (is_active, deleted_at, display_order);--> statement-breakpoint

CREATE TABLE IF NOT EXISTS archidoc_technical_lot_catalogue (
  singleton_key integer PRIMARY KEY DEFAULT 1,
  revision bigint NOT NULL,
  changed_at timestamp NOT NULL,
  source_base_url text,
  synced_at timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT archidoc_technical_lot_catalogue_singleton_chk CHECK (singleton_key = 1),
  CONSTRAINT archidoc_technical_lot_catalogue_revision_chk CHECK (revision >= 0)
);--> statement-breakpoint

ALTER TABLE planning_revisions
  ADD COLUMN IF NOT EXISTS archidoc_technical_lot_id varchar(255);--> statement-breakpoint

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
     WHERE conname = 'planning_revisions_archidoc_technical_lot_id_fk'
  ) THEN
    ALTER TABLE planning_revisions
      ADD CONSTRAINT planning_revisions_archidoc_technical_lot_id_fk
      FOREIGN KEY (archidoc_technical_lot_id)
      REFERENCES archidoc_technical_lots(archidoc_id)
      ON UPDATE RESTRICT
      ON DELETE RESTRICT;
  END IF;
END
...
CREATE INDEX IF NOT EXISTS planning_revisions_archidoc_technical_lot_id_idx
  ON planning_revisions (archidoc_technical_lot_id);--> statement-breakpoint

-- Re-assert the latest approved-revision guard with the new master-lot
-- reference included among the immutable commercial facts.
          ...
          SELECT 1
           ...
           WHERE d.id = NEW.promoted_devis_id
             ...
             AND d.ai_extracted_data IS NOT DISTINCT FROM NULLIF(
               NEW.approved_snapshot #> '{source,rawExtraction}',
               'null'::jsonb
             )
             AND d.validation_warnings IS NOT DISTINCT FROM NULLIF(
               NEW.approved_snapshot #> '{source,warnings}',
               'null'::jsonb
             )
             AND (
               SELECT count(*)
