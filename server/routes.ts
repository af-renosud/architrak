...
import type { InsertFeeEntry, FeeEntry } from "@shared/schema";
import { db } from "./db";
...
export async function registerRoutes(
  ...
  app.post("/api/invoices/:id/approve", async (req, res) => {
    try {
      ...
      if (inv.status !== "pending") return res.status(400).json({ message: `Invoice is already ${inv.status}, cannot approve` });
  ...
  app.post("/api/fee-entries/:id/mark-invoiced", async (req, res) => {
    const entryId = Number(req.params.id);
    const { pennylaneInvoiceRef } = req.body ?? {};

    const feesList = await db.select().from(feeEntries).where(eq(feeEntries.id, entryId));
    if (!feesList.length) return res.status(404).json({ message: "Fee entry not found" });
    const targetEntry = feesList[0];
    if (targetEntry.status !== "pending") return res.status(400).json({ message: "Entry is not in pending status" });
