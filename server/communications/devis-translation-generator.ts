...
import { renderCostAnalysisHtml, COST_ANALYSIS_PDF_CSS } from "./cost-analysis-renderer";
...
interface BuildHtmlInput {
  ...
  /** Pre-rendered (whitelisting serializer) CONFIRMED analysis, or null. */
  costAnalysisHtml: string | null;
}
...
<body>
  ...
  ${costAnalysisHtml ? `<div class="ca-section">
    <div class="ca-header">
      <div class="ca-title">Cost Analysis &amp; Value Engineering</div>
      <div class="ca-subtitle">${escapeHtml(devis.devisCode)} — ${escapeHtml(contractor.name)}</div>
    </div>
    ${costAnalysisHtml}
    <div class="ca-disclaimer">
      <strong>Non-contractual analysis:</strong> This cost analysis is provided for information only and is not part of the contractual quotation.
      The original French devis remains the sole legally binding reference for scope, quantities and amounts.
    </div>
  </div>` : ""}
...
export async function generateDevisTranslationPdf(
  ...
  // Loaded AFTER the version snapshot — analysis confirm/edit/remove bumps
  // contexts_version, so the version-guarded publish below covers a
  // concurrent analysis change exactly like a context save.
  const costAnalysisDoc = await getConfirmedCostAnalysisDocument(devisId);
  const costAnalysisHtml = costAnalysisDoc ? renderCostAnalysisHtml(costAnalysisDoc) : null;
