...
- KNOWN GAP: the seal path links sources with onConflictDoNothing and no advisory lock — a manual cert sealed for the same contractor can still claim an already-certified invoice (whole-contractor generator scope). Cross-path exclusivity is not yet atomic.
